...
class DownloadAuditTrailConnectorPropertyTest {
    ...
    @Property
    void should_accept_null_signerIdsJson() {
        var config = YousignConfiguration.builder().apiKey("key").baseUrl("url").signatureRequestId("req").signerIdsJson(null).build();
        assertThat(config.getSignerIdsJson()).isNull();
    }

    @Property
    void should_preserve_signerIdsJson(@ForAll @StringLength(min = 1, max = 200) String json) {
        var config = YousignConfiguration.builder().apiKey("key").signerIdsJson(json).build();
        assertThat(config.getSignerIdsJson()).isEqualTo(json);
    }
